 ...
 * @author wilqor
 */
final class SyncVoteCreatesTask implements RemoteTask {
    private final VotesService votesService;
    private final ErrorResponseHandler errorResponseHandler;
    private final String userLogin;

    SyncVoteCreatesTask(VotesService votesService, ErrorResponseHandler errorResponseHandler, String userLogin) {
    ...
    public void perform() throws IOException, UnauthorizedException, ErrorResponseException {
        List<Vote> createdVotes = SQLite.select()
                .from(Vote.class)
                .where(Vote_Table.voter.eq(userLogin))
                .and(Vote_Table.remoteId.eq(Vote.ID_NOT_SET))
                .queryList();
        for (Vote created : createdVotes) {
            Response<Void> createdResponse = votesService.addVote(userLogin, created.asNewVoteDto()).execute();
